...
public class MCreatorDeactivateShieldPower extends Elementstest_procedurcese.ModElement {
	private KeyBinding keys;

	public MCreatorDeactivateShieldPower(Elementstest_procedurcese instance) {
		super(instance, 24);
	}
	...
	public void init(FMLInitializationEvent event) {
		keys = new KeyBinding("key.mcreator.deactivateshieldpower", Keyboard.KEY_B, "key.categories.misc");
	...
	public void onKeyInput(InputEvent.KeyInputEvent event) {
		if (!FMLClientHandler.instance().isGUIOpen(GuiChat.class)) {
			if (org.lwjgl.input.Keyboard.isKeyDown(keys.getKeyCode())) {
				test_procedurcese.PACKET_HANDLER.sendToServer(new KeyBindingPressedMessage());
	...
	private static void pressAction(EntityPlayer entity) {
			...
			return;
		{
			...
			MCreatorDeactivateShieldPowerOnKeyPressed.executeProcedure($_dependencies);
		}
	}
}
